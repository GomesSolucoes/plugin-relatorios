...
export class FechamentoLivroCaixaComponent implements OnInit {
  ...
  verificarQuantidadeDiasDiferencaDatas() {
    let dataInicial: string =
      this.filtroFechamentoLivroCaixaForm.get("dataInicio").value;
    let dataFinal: string =
      this.filtroFechamentoLivroCaixaForm.get("dataFinal").value;
    ...
    if (this.filtroFechamentoLivroCaixaForm.valid) {
      ...
      let daysDiff: number = this.utilCore.utilFuncoes.getDiffDays(
        dataInicial,
        dataFinal
      );
      if (daysDiff > this.quantidadeDiasParaConsultarFechamentoLivroCaixa) {
        this.utilCore.utilMensagens.mostrarMensagemAlerta(
          this.mensagensPTBRRelatorios.mensagens.label
            .quantidadeDiasMaiorQueParamentro +
            this.quantidadeDiasParaConsultarFechamentoLivroCaixa
        );
      } else {
  ...
  consultarQuantidadeDiasParaConsultarFechamentoLivroCaixa() {
    ...
    this.utilCore.utilService
      .service(
        this.serviceConfigCore.serviceInterfaceCore.consultarValorParametroById
      )
      .then((quantidadeDias: string) => {
        this.quantidadeDiasParaConsultarFechamentoLivroCaixa =
  ...
  consultarfechamentoLivroCaixaByFiltro() {
    let dataInicial: string;/* =
      this.utilCore.utilFuncoes.jsNativeDateFormatToBrasillianFormat(
        this.filtroFechamentoLivroCaixaForm.get("dataInicio").value
      );*/
    let dataFinal: string ;/*=
      this.utilCore.utilFuncoes.jsNativeDateFormatToBrasillianFormat(
    ...
    this.serviceConfigRelatorios.serviceInterfaceFinanceiro.consultarfechamentoLivroCaixaByFiltro.urlParam =
      "?dataFinal=" +
      dataFinal +
      "&dataInicial=" +
      dataInicial +
    ...
    this.utilCore.utilService
      .service(
        this.serviceConfigRelatorios.serviceInterfaceFinanceiro
          .consultarfechamentoLivroCaixaByFiltro
      )
      .then((listaFechamentoLivroCaixa: FechamentoLivroCaixa[]) => {
        this.labelFechamento = [];
        this.valoresFechamento = [];
        listaFechamentoLivroCaixa.forEach(
          (fechamentoLivroCaixa: FechamentoLivroCaixa) => {
            this.labelFechamento.push(fechamentoLivroCaixa.dataAbertura);
            this.valoresFechamento.push(
              Number(fechamentoLivroCaixa.valorFechamentoCaixa).toFixed(2)
            );
          }
        );
        this.typeGraph = null;
      ...
      .catch((err) => {
        console.error(err);
        this.utilCore.utilMensagens.mostrarMensagemAlerta(
          this.mensagensPTBRRelatorios.mensagens.label.error
            .erroAoConsultarFechamentoLivroCaixa + err
        );
      });
  }
